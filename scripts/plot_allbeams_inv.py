...
#           print("Image saved to file %s" % (self.outfilename))
    ...
    def draw(self):
        ...
        f0 = files[0]
#        f0.foff = -f0.foff # MS TEST
        f0.fch1 = f0.fch1 - f0.foff*self.nfreq
        print("DEBUG : f0.foff = %.6f , f0.fch1 = %.8f" % (f0.foff,f0.fch1))
        self.beams = beams
        ...
        ntimes, nbeams, nfreq = beams.shape
        print("DEBUG : nfreq = %d vs. self.nfreq = %d" % (nfreq,self.nfreq))
        mjdstart = f0.tstart
